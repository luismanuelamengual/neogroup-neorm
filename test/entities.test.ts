...
// Reusable scope object: filters only active users
class ActiveUserScope implements Scope<ScopedUser> {
  apply(query: EntityQuery<ScopedUser>): void {
    query.where('active', 1)
  }
}

// Entity with a scope registered via Scope class in booted()
@Entity({ table: 'users' })
class ScopedUser extends BaseEntity {
  ...
  protected static booted(): void {
    ScopedUser.addGlobalScope(new ActiveUserScope())
  }
}

// Entity with two scopes: one via Scope class, one via inline callback
@Entity({ table: 'users' })
class MultiScopedUser extends BaseEntity {
  @Column({ primaryKey: true, autoGenerated: true })
  id!: number
  ...
  protected static booted(): void {
    MultiScopedUser.addGlobalScope(new ActiveUserScope())
    MultiScopedUser.addGlobalScope('youngAdult', (query: EntityQuery<any>) => {
      query.where('age', '<', 30)
    })
  }
}
...
describe('Entities (Active Record — BaseEntity)', () => {
  ...
  describe('Global Scopes', () => {
    ...
    it('el scope global filtra automáticamente los resultados', async () => {
      const users = await ScopedUser.get()

      expect(users).toHaveLength(2)
      expect(users.map((u) => u.name).sort()).toEqual(['Alice', 'Bob'])
    })

    it('withoutGlobalScope desactiva un scope por nombre', async () => {
      const users = await ScopedUser.withoutGlobalScope('ActiveUserScope').get()

      expect(users).toHaveLength(3)
    })

    it('withoutGlobalScopes desactiva todos los scopes', async () => {
      const users = await ScopedUser.withoutGlobalScopes().get()

      expect(users).toHaveLength(3)
    })

    it('withoutGlobalScopes combinado con where adicional', async () => {
      const users = await ScopedUser.withoutGlobalScopes().where('active', 0).get()

      expect(users).toHaveLength(1)
      expect(users[0].name).toBe('Charlie')
    })

    it('el scope global se aplica en first()', async () => {
      const user = await ScopedUser.where('name', 'Charlie').first()

      expect(user).toBeNull()
    })

    it('el scope global se aplica en count()', async () => {
      const total = await ScopedUser.count()

      expect(total).toBe(2)
    })

    it('el scope global se aplica en find()', async () => {
      // Insertar usuario inactivo y buscarlo por id — el scope debe ocultarlo
      await source.execute(`INSERT INTO users (name, email, age, active, countryId) VALUES ('Inactive', 'i@i.com', 40, 0, NULL)`)
      const row = await source.table('users').where('name', 'Inactive').first()
      const found = await ScopedUser.find(row!.id)

      expect(found).toBeNull()
    })

    it('múltiples scopes se combinan con AND', async () => {
      // MultiScopedUser tiene active=1 AND age<30 → solo Bob (25)
      const users = await MultiScopedUser.get()

      expect(users).toHaveLength(1)
      expect(users[0].name).toBe('Bob')
    })

    it('withoutGlobalScope desactiva solo el scope indicado', async () => {
      // Sin youngAdult → active=1 → Alice (30) y Bob (25)
      const users = await MultiScopedUser.withoutGlobalScope('youngAdult').get()

      expect(users).toHaveLength(2)
      expect(users.map((u) => u.name).sort()).toEqual(['Alice', 'Bob'])
    })

    it('addGlobalScope con Scope object usa el nombre de la clase como key', async () => {
      // Desactivar ambos scopes por nombre → los 3 registros
      const users = await MultiScopedUser.withoutGlobalScope('ActiveUserScope').withoutGlobalScope('youngAdult').get()

      expect(users).toHaveLength(3)
    })

    it('el scope global se aplica también al encadenar where()', async () => {
      // age>=28 AND active=1 → Alice (30) cumple; Charlie (35) excluido por scope
      const users = await ScopedUser.where('age', '>=', 28).get()
